...
<body data-spy="scroll" data-target=".navbar" data-offset="40" id="home">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["presentationFile"])) {
        ...
        // Write Conference Name to header.txt file
        if (isset($_POST['name'])) {
            $conference_name = $_POST['name'];
            if (!empty($conference_name)) {
                $target_file = $target_dir . "header.txt"; // Set file name as header.txt
                file_put_contents($target_file, $conference_name);
            }
        }

        $target_file = $target_dir . "about_conference.txt"; // Set file name as about_conference.txt
        $uploadOk = 1;
        $fileType = strtolower(pathinfo($_FILES["presentationFile"]["name"], PATHINFO_EXTENSION));

        // Only allow certain file types
        $allowedExtensions = array("pdf", "docx", "pptx", "ppt", "txt"); // accepted file extensions
        if (empty($conference_name)) {
            echo "Conference Name cannot be empty.";
        } elseif (empty($_FILES['presentationFile']['tmp_name'])) { // If file not selected
